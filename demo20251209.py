...
with DAG(
    ...
    def get_latest_rs_line():
        ...
        var_namespace  = Variable.get("NAME_SPACE")
        var_ci_cd_pipeline_name  = Variable.get("CI_CD_PIPELINE_NAME")# ft-ml-pipeline
        rs_list = api.list_namespaced_replica_set(namespace=var_namespace)
        # Filter all rs of deployment
        filtered = [rs for rs in rs_list.items if var_ci_cd_pipeline_name in rs.metadata.name]
